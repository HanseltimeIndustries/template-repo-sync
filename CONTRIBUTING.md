...
For bugfixes, please make sure to include any relevant output or context and make sure to describe
exactly which package is failing from the many in the monorepo.
...
<!-- TODO - Set up monorepo mkdocs patterns
### Documentation
...
When opening a PR from a fork, please link the issue that you are solving (and if one does not exist, please make one).
...
1. Ensure you are on node >=24 and that corepack is enabled: `corepack enable`
2. Explicitly run `yarn install`

### Using Changesets

This repo uses the [changesets tool](https://github.com/changesets/changesets) to describe changes to packages and track their releases.

Practically, if you are making any changes to a package in the monorepo, you need to:

1. Create a changeset and fill in the appropriate information `yarn changeset`
2. Commit the file as part of your PR

Once the file has merged to main, your change will trigger a release.
...
```shell
# Alias for running test in all packages
yarn test

# If you want to just run a test in one package
