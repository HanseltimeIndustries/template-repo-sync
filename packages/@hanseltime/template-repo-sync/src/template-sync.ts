...
import type { TemplateCheckoutDriverFn } from "./checkout-drivers";
import { gitCheckout } from "./checkout-drivers";
import type { TemplateCloneDriverFn } from "./clone-drivers";
import { gitClone } from "./clone-drivers/git-clone";
import type { DiffResult, TemplateDiffDriverFn } from "./diff-drivers";
import { gitDiff } from "./diff-drivers";
...
import type { TemplateRefDriverFn } from "./ref-drivers/types";
import type { Config, FileOperation, LocalConfig } from "./types";
...
export async function templateSync(
	...
	if (options.updateAfterRef) {
		...
		if (existsSync(localConfigPath)) {
			...
			writeFileSync(
				localConfigPath,
				commentJSON.stringify({ afterRef: _ref }, null, 4),
			);
